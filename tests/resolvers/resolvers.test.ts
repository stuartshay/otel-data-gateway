...
describe('garmin segment resolvers', () => {
  ...
  it('extracts id before calling garminSegmentEffortSeries', async () => {
    const series = {
      activity_id: 'a',
      effort_start: '2026-07-05T10:30:00',
      effort_end: '2026-07-05T10:32:00',
      bin_count: 100,
      bins: [{ index: 0, fraction: 0.005, speed_kmh: 18.4, heart_rate: 132 }],
    };
    const ctx = contextWith({ getGarminSegmentEffortSeries: mockAsync(series) });
    const args = {
      id: 1,
      activity_id: 'a',
      effort_start: '2026-07-05T10:30:00',
    ...
    const result = await runResolver(garminResolvers.Query.garminSegmentEffortSeries, args, ctx);

    expect(ctx.dataSources.otelAPI.getGarminSegmentEffortSeries).toHaveBeenCalledWith(1, {
      activity_id: 'a',
      effort_start: '2026-07-05T10:30:00',
      effort_end: '2026-07-05T10:32:00',
      bins: 100,
